...
extends Module {
    ...
    public Setting<String> command = this.register(new Setting<String>("Command", "QuartzHack.cc"));
    public Setting<Boolean> rainbowPrefix = this.register(new Setting<Boolean>("RainbowPrefix", false));
    public Setting<TextUtil.Color> bracketColor = this.register(new Setting<TextUtil.Color>("BColor", TextUtil.Color.DARK_PURPLE));
    public Setting<TextUtil.Color> commandColor = this.register(new Setting<TextUtil.Color>("CColor", TextUtil.Color.DARK_PURPLE));
    ...
    @Override
    public void onLoad() {
        Banzem.commandManager.setClientMessage(this.getCommandMessage());
    }
        ...
        if (event.getStage() == 2) {
            ...
            if (event.getSetting() != null && this.equals(event.getSetting().getFeature())) {
                if (event.getSetting().equals(this.holeThread)) {
                    Banzem.holeManager.settingChanged();
                }
                Banzem.commandManager.setClientMessage(this.getCommandMessage());
            }
        }
