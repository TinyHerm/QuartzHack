...
extends Module {
    ...
    private void renderNameTag(EntityPlayer player, double x, double y, double z, float delta) {
        ...
        Entity camera = Util.mc.func_175606_aa();
        assert (camera != null);
    ...
    private String getDisplayTag(EntityPlayer player) {
        ...
        if (this.ping.getValue().booleanValue()) {
            try {
                int responseTime = Objects.requireNonNull(Util.mc.func_147114_u()).func_175102_a(player.func_110124_au()).func_178853_c();
        ...
        if (this.totemPops.getValue().booleanValue()) {
            popStr = popStr + Banzem.totemPopManager.getTotemPopString(player);
        }
        String idString = "";
