...
extends Module {
    ...
    public void onRender2D(Render2DEvent event) {
        ...
        String grayString = this.grayNess.getValue() != false ? String.valueOf(ChatFormatting.GRAY) : "";
        switch (this.watermark.getValue()) {
            case BANZEM: {
                this.renderer.drawString("QuartzHack.cc " + (this.modeVer.getValue() != false ? "0.3.0-b22" : ""), 2.0f, 2.0f, this.rolling.getValue() != false && this.rainbow.getValue() != false ? this.colorMap.get(2) : this.color, true);
            }
        }
        ...
        if (this.renderingUp.getValue().booleanValue()) {
            ...
            if (this.serverBrand.getValue().booleanValue()) {
                text2 = grayString + "Server brand " + ChatFormatting.WHITE + Banzem.serverManager.getServerBrand();
            ...
            if (this.potions.getValue().booleanValue()) {
                for (PotionEffect effect : Banzem.potionManager.getOwnPotions()) {
                    ...
                    int n32 = height - 2;
                    renderer3.drawString(text7, x3, n32 - (k += 10), this.rolling.getValue() != false && this.rainbow.getValue() != false ? this.colorMap.get(height - k) : (this.altPotionsColors.getValue() != false ? this.potionColorMap.get(effect.func_188419_a()).getRGB() : this.color), true);
                }
            }
            if (this.speed.getValue().booleanValue()) {
                text2 = grayString + "Speed " + ChatFormatting.WHITE + Banzem.speedManager.getSpeedKpH() + " km/h";
            ...
            if (this.time.getValue().booleanValue()) {
                text2 = grayString + "Time " + ChatFormatting.WHITE + new SimpleDateFormat("h:mm a").format(new Date());
            ...
            if (this.tps.getValue().booleanValue()) {
                String text22 = grayString + "TPS " + ChatFormatting.WHITE + Banzem.serverManager.getTPS();
            ...
            text = grayString + "Ping " + ChatFormatting.WHITE + (ServerModule.getInstance().isConnected() ? ServerModule.getInstance().getServerPing() : (long)Banzem.serverManager.getPing()) + (this.MS.getValue() != false ? "ms" : "");
            ...
            if (this.potions.getValue().booleanValue()) {
                for (PotionEffect effect : Banzem.potionManager.getOwnPotions()) {
                    text3 = this.altPotionsColors.getValue() != false ? Banzem.potionManager.getPotionString(effect) : Banzem.potionManager.getColoredPotionString(effect);
                    this.renderer.drawString(text3, width - (this.renderer.getStringWidth(text3) + 2), 2 + k++ * 10, this.rolling.getValue() != false && this.rainbow.getValue() != false ? this.colorMap.get(2 + k * 10) : (this.altPotionsColors.getValue() != false ? this.potionColorMap.get(effect.func_188419_a()).getRGB() : this.color), true);
                }
            }
            if (this.speed.getValue().booleanValue()) {
                text2 = grayString + "Speed " + ChatFormatting.WHITE + Banzem.speedManager.getSpeedKpH() + " km/h";
            ...
            if (this.tps.getValue().booleanValue()) {
                String text23 = grayString + "TPS " + ChatFormatting.WHITE + Banzem.serverManager.getTPS();
            ...
            fpsText = grayString + "FPS " + ChatFormatting.WHITE + Minecraft.field_71470_ab;
            text = grayString + "Ping " + ChatFormatting.WHITE + Banzem.serverManager.getPing();
            if (this.renderer.getStringWidth(text) > this.renderer.getStringWidth(fpsText)) {
                if (this.ping.getValue().booleanValue()) {
                    this.renderer.drawString(text, width - (this.renderer.getStringWidth(text) + 2), 2 + k++ * 10, this.rolling.getValue() != false && this.rainbow.getValue() != false ? this.colorMap.get(2 + k * 10) : this.color, true);
                }
    ...
    public void renderGreeter() {
        ...
        switch (this.greeter.getValue()) {
            ...
            case LONG: {
                text = text + "Welcome to QuartzHack.cc " + HUD.mc.field_71439_g.getDisplayNameString() + " :^)";
                break;
            }
            case CUSTOM: {
    ...
    public void renderLag() {
        ...
        if (Banzem.serverManager.isServerNotResponding()) {
            String text = (this.lag.getValue() == LagNotify.GRAY ? ChatFormatting.GRAY : ChatFormatting.RED) + "Server not responding: " + MathUtil.round((float)Banzem.serverManager.serverRespondingTime() / 1000.0f, 1) + "s.";
