...
extends Module {
    ...
    public AutoGG() {
        ...
        File file = new File("QuartzHack/autogg.txt");
        if (!file.exists()) {
            try {
                file.createNewFile();
            }
    ...
    public void onSendAttackPacket(PacketEvent.Send event) {
        ...
        if (event.getPacket() instanceof CPacketUseEntity && (packet = (CPacketUseEntity)event.getPacket()).func_149565_c() == CPacketUseEntity.Action.ATTACK && packet.func_149564_a((World)AutoGG.mc.field_71441_e) instanceof EntityPlayer && !Banzem.friendManager.isFriend((EntityPlayer)packet.func_149564_a((World)AutoGG.mc.field_71441_e))) {
            this.targets.put((EntityPlayer)packet.func_149564_a((World)AutoGG.mc.field_71441_e), 0);
        }
    }

    public void loadMessages() {
        this.messages = FileManager.readTextFileAllLines("QuartzHack/autogg.txt");
    }
