...
extends Module {
    ...
    public void onPacketReceive(PacketEvent.Receive event) {
        ...
        if (ServerModule.getInstance().isConnected() && event.getPacket() instanceof SPacketSetSlot && (packet = (SPacketSetSlot)event.getPacket()).func_149173_d() == -1 && packet.func_149175_c() != -1) {
    ...
    private boolean noNearbyPlayers() {
        return this.mode == Mode.CRYSTALS && Offhand.mc.field_71441_e.field_73010_i.stream().noneMatch(e -> e != Offhand.mc.field_71439_g && !Banzem.friendManager.isFriend((EntityPlayer)e) && Offhand.mc.field_71439_g.func_70032_d((Entity)e) <= this.cTargetDistance.getValue().floatValue());
    }
