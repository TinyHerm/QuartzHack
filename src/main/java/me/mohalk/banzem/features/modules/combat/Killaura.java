...
extends Module {
    ...
    public void onUpdateWalkingPlayerEvent(UpdateWalkingPlayerEvent event) {
        if (event.getStage() == 0 && this.rotate.getValue().booleanValue()) {
            if (this.stay.getValue().booleanValue() && target != null) {
                Banzem.rotationManager.lookAtEntity(target);
            }
            this.doKillaura();
        }
    }
    ...
    private void doKillaura() {
        ...
        int n = this.delay.getValue() == false || EntityUtil.holding32k((EntityPlayer)Killaura.mc.field_71439_g) && this.teekaydelay.getValue() == false ? 0 : (wait = (wait = (int)((float)DamageUtil.getCooldownByWeapon((EntityPlayer)Killaura.mc.field_71439_g) * (this.tps.getValue() != false ? Banzem.serverManager.getTpsFactor() : 1.0f))));
        if (!this.timer.passedMs(wait) || !this.eating.getValue().booleanValue() && Killaura.mc.field_71439_g.func_184587_cr() && (!Killaura.mc.field_71439_g.func_184592_cb().func_77973_b().equals(Items.field_185159_cQ) || Killaura.mc.field_71439_g.func_184600_cs() != EnumHand.OFF_HAND)) {
            return;
        }
        ...
        if (this.teleport.getValue().booleanValue()) {
            Banzem.positionManager.setPositionPacket(Killaura.target.field_70165_t, EntityUtil.canEntityFeetBeSeen(target) ? Killaura.target.field_70163_u : Killaura.target.field_70163_u + (double)target.func_70047_e(), Killaura.target.field_70161_v, true, true, this.lagBack.getValue() == false);
        }
