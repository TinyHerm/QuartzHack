...
extends Module {
    ...
    private void doHoleFill() {
        ...
        if (this.midSafeHoles.getValue().booleanValue()) {
            object2 = Banzem.holeManager.getMidSafety();
            object = object2;
            synchronized (object) {
                targets = new ArrayList<BlockPos>(Banzem.holeManager.getMidSafety());
            }
        }
        object2 = Banzem.holeManager.getHoles();
        object = object2;
        synchronized (object) {
            targets = new ArrayList<BlockPos>(Banzem.holeManager.getHoles());
        }
    ...
    private boolean check() {
        ...
        this.switchItem(true);
        if (!this.freecam.getValue().booleanValue() && Banzem.moduleManager.isModuleEnabled(Freecam.class)) {
            return true;
        }
