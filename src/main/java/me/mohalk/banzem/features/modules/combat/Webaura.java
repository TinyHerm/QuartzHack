...
extends Module {
    ...
    private boolean check() {
        ...
        return this.target == null || Banzem.moduleManager.isModuleEnabled("Freecam") && this.freecam.getValue() == false || !this.timer.passedMs(this.delay.getValue().intValue()) || this.switchMode.getValue() == InventoryUtil.Switch.NONE && Webaura.mc.field_71439_g.field_71071_by.field_70461_c != InventoryUtil.findHotbarBlock(BlockWeb.class);
    }
    ...
    private EntityPlayer getTarget(double range, boolean trapped) {
        ...
        for (EntityPlayer player : Webaura.mc.field_71441_e.field_73010_i) {
            if (EntityUtil.isntValid((Entity)player, range) || trapped && player.field_70134_J || EntityUtil.getRoundedBlockPos((Entity)Webaura.mc.field_71439_g).equals((Object)EntityUtil.getRoundedBlockPos((Entity)player)) && this.antiSelf.getValue().booleanValue() || Banzem.speedManager.getPlayerSpeed(player) > this.speed.getValue()) continue;
            if (target == null) {
                target = player;
