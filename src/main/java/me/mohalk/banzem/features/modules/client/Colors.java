...
extends Module {
    ...
    public void onTick() {
        ...
        if (ClickGui.getInstance().colorSync.getValue().booleanValue()) {
            Banzem.colorManager.setColor(INSTANCE.getCurrentColor().getRed(), INSTANCE.getCurrentColor().getGreen(), INSTANCE.getCurrentColor().getBlue(), ClickGui.getInstance().hoverAlpha.getValue());
        }
    }
