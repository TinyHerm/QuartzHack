...
extends Feature {
    ...
    public static void sendMessage(String message, boolean notification) {
        Command.sendSilentMessage(Banzem.commandManager.getClientMessage() + " \u00a7r" + message);
        if (notification) {
            Banzem.notificationManager.addNotification(message, 3000L);
        }
    }
    ...
    public static String getCommandPrefix() {
        return Banzem.commandManager.getPrefix();
    }
