...
extends Command {
    ...
    public void execute(String[] commands) {
        ...
        if (commands.length == 1) {
            ...
            for (Module.Category category : Banzem.moduleManager.getCategories()) {
                String modules = category.getName() + ": ";
                for (Module module : Banzem.moduleManager.getModulesByCategory(category)) {
        ...
        Module module = Banzem.moduleManager.getModuleByDisplayName(commands[0]);
        if (module == null) {
            module = Banzem.moduleManager.getModuleByName(commands[0]);
            if (module == null) {
