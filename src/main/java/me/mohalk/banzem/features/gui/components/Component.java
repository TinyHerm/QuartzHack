...
extends Feature {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (this.open) {
            ...
            if (ClickGui.getInstance().outline.getValue().booleanValue()) {
                if (ClickGui.getInstance().rainbowRolling.getValue().booleanValue()) {
                    ...
                    GlStateManager.func_179103_j((int)7425);
                    GL11.glBegin((int)2);
                    Color outlineColor = ClickGui.getInstance().colorSync.getValue() != false ? new Color(Colors.INSTANCE.getCurrentColorHex()) : new Color(Banzem.colorManager.getColorAsIntFullAlpha());
        ...
        Banzem.textManager.drawStringWithShadow(this.getName(), (float)this.x + 3.0f, (float)this.y - 4.0f - (float)PhobosGui.getClickGui().getTextOffset(), -1);
        if (this.open) {
