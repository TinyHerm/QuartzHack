...
implements Util {
    ...
    public static List<Vec3d> getUnsafeBlocks(Entity entity, int height, boolean floor, boolean face) {
        return EntityUtil.getUnsafeBlocksFromVec3d(entity.func_174791_d().func_72441_c(0.0, 0.125, 0.0), height, floor, face);
    }
    ...
    public static boolean isCrystalAtFeet(EntityEnderCrystal crystal, double range) {
        for (EntityPlayer player : EntityUtil.mc.field_71441_e.field_73010_i) {
            if (EntityUtil.mc.field_71439_g.func_70068_e((Entity)player) > range * range || Banzem.friendManager.isFriend(player)) continue;
    ...
    public static List<EntityPlayer> getNearbyPlayers(double d) {
        ...
        list.removeIf(entityPlayer -> Banzem.friendManager.isFriend(entityPlayer.func_70005_c_()));
        return list;
    }
    ...
    public static boolean isntValid(Entity entity, double range) {
        return entity == null || EntityUtil.isDead(entity) || entity.equals((Object)EntityUtil.mc.field_71439_g) || entity instanceof EntityPlayer && Banzem.friendManager.isFriend(entity.func_70005_c_()) || EntityUtil.mc.field_71439_g.func_70068_e(entity) > MathUtil.square(range);
    }
    ...
    public static Color getColor(Entity entity, int red, int green, int blue, int alpha, boolean colorFriends) {
        ...
        if (entity instanceof EntityPlayer) {
            if (colorFriends && Banzem.friendManager.isFriend((EntityPlayer)entity)) {
                color = new Color(0.33333334f, 1.0f, 1.0f, (float)alpha / 255.0f);
            }
            Killaura killaura = Banzem.moduleManager.getModuleByClass(Killaura.class);
    ...
    public static Map<String, Integer> getTextRadarPlayers() {
        ...
        for (EntityPlayer player : EntityUtil.mc.field_71441_e.field_73010_i) {
            ...
            output.put(healthSB + " " + (Banzem.friendManager.isFriend(player) ? "\u00a7b" : "\u00a7r") + player.func_70005_c_() + " " + distanceSB + " \u00a7f" + Banzem.totemPopManager.getTotemPopString(player) + Banzem.potionManager.getTextRadarPotion(player), (int)EntityUtil.mc.field_71439_g.func_70032_d((Entity)player));
