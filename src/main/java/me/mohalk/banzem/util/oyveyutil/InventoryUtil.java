...
import me.mohalk.banzem.util.oyveyutil.Util;
...
implements Util {
    ...
    public static List<Integer> getItemInventory(Item item) {
        ...
        for (int i = 9; i < 36; ++i) {
            Item target = InventoryUtil.mc.field_71439_g.field_71071_by.func_70301_a(i).func_77973_b();
        ...
        switch (mode) {
            ...
            case SILENT: {
                if (!back && !switchedItem) {
                    ...
                    switchedItemSwitched[0] = false;
                    Banzem.inventoryManager.recoverSilent(lastHotbarSlot);
                }
                switchedItemSwitched[1] = true;
                break;
            }
    ...
    public static int findBlockSlotInventory(Class clazz, boolean offHand, boolean withXCarry) {
        ...
        if (slot == -1 && withXCarry) {
            for (int i = 1; i < 5; ++i) {
                ...
                if (!(craftingStackItem instanceof ItemBlock) || !clazz.isInstance(block = ((ItemBlock)craftingStackItem).func_179223_d())) continue;
                slot = i;
            }
        }
        return slot;
